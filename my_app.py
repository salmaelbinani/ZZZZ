...
try:
    ...
    expiration_date = datetime(2024, 9, 10, 14, 0, tzinfo=pytz.UTC)  # September 15, 2024, at 11:59 PM UTC
    ...
    if current_date > expiration_date:
        ...
        if st.button("Separate"):
            if uploaded_file is not None and uploaded_circuit_file is not None:
                ...
                circuit_file_path = os.path.join(uploads_dir, uploaded_circuit_file.name)
                with open(circuit_file_path, "wb") as f:
                    f.write(uploaded_circuit_file.getbuffer())

                # Read and modify the separation notebook
                notebook_path = 'separation.ipynb'  # Update this path if necessary
                
                with open(notebook_path, 'r', encoding='utf-8') as f:
                    nb = nbformat.read(f, as_version=4)
                
                # Search and replace the paths of the Excel files in the notebook
        ...
        if st.button("Integrate"):
            if uploaded_file is not None and uploaded_circuit_file is not None:
                ...
                notebook_integration_path = 'integrationfinal1.ipynb'  # Update this path if necessary
                
                with open(notebook_integration_path, 'r', encoding='utf-8') as f:
                    nb_integration = nbformat.read(f, as_version=4)
                
                # Search and replace the paths of the Excel files in the integration notebook
